...
public class P054 {
	public static void main(String args[]) {
		...
		for (int i = 0; i < hands.length; i++) {
			...
			for (int j = 0; j < 5; j++) {
				cards1[j] = new Card(hands[i][j]);
				cards2[j] = new Card(hands[i][j + 5]);
			}
			Arrays.sort(cards1);
			Arrays.sort(cards2);
			Rank rank1 = getRank(cards1);
			Rank rank2 = getRank(cards2);
			int rankCompare = rank1.comareTo(rank2);
			if (rankCompare > 0) {
				player1WinCount++;
			} else if (rankCompare == 0) {
				for (int j = 4; j >= 0; j--) {
					if (cards1[j].value != cards2[j].value) {
						if (cards1[j].value > cards2[j].value) {
							player1WinCount++;
						}
						break;
	...
	static Rank getRank(Card cards[]) {
		if (cards[0].value == 10 && cards[1].value == 11
				&& cards[2].value == 12 && cards[3].value == 13
				&& cards[4].value == 14 && cards[0].suit == cards[1].suit
				&& cards[0].suit == cards[2].suit
				&& cards[0].suit == cards[3].suit
				&& cards[0].suit == cards[4].suit) {
			return new Rank(10, 14, -1);
		}
		...
		if ((cards[0].value == cards[1].value
			...
			return new Rank(7, cards[2].value, -1);
		}
		if (cards[0].suit == cards[1].suit && cards[0].suit == cards[2].suit
				&& cards[0].suit == cards[3].suit
				&& cards[0].suit == cards[4].suit) {
			return new Rank(6, cards[4].value, -1);
		}
		if (cards[0].value + 1 == cards[1].value
				&& cards[1].value + 1 == cards[2].value
				&& cards[2].value + 1 == cards[3].value
				&& cards[3].value + 1 == cards[4].value) {
			return new Rank(5, cards[4].value, -1);
		}
		if ((cards[0].value == cards[1].value && cards[1].value == cards[2].value)
				|| (cards[1].value == cards[2].value && cards[2].value == cards[3].value)
				|| (cards[2].value == cards[3].value && cards[3].value == cards[4].value)) {
			return new Rank(4, cards[2].value, -1);
		}
		if ((cards[0].value == cards[1].value && cards[2].value == cards[3].value)
				|| (cards[0].value == cards[1].value && cards[3].value == cards[4].value)
				|| (cards[1].value == cards[2].value && cards[3].value == cards[4].value)) {
			return new Rank(3, cards[3].value, cards[1].value);
		}
		int pair;
		if ((pair = cards[0].value) == cards[1].value
				|| (pair = cards[1].value) == cards[2].value
				|| (pair = cards[2].value) == cards[3].value
				|| (pair = cards[3].value) == cards[4].value) {
			return new Rank(2, pair, -1);
		}
		return new Rank(1, cards[4].value, -1);
	}
}
...
class Rank {
	...
	Rank(int theRank, int theHighestValue1, int theHighestValue2) {
		this.rank = theRank;
		this.highestValue1 = theHighestValue1;
		this.highestValue2 = theHighestValue2;
	}

	int comareTo(Rank another) {
		return (this.rank != another.rank) ? this.rank - another.rank
				: (this.highestValue1 != another.highestValue1) ? this.highestValue1
						- another.highestValue1
						: this.highestValue2 - another.highestValue2;
	}
}